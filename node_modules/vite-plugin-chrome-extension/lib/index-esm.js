import path, { relative, dirname, resolve, join, parse, basename } from 'path';
...
function getOutputFilenameFromChunk(sourceFileName, chunks) {
    ...
    return slash(_optionalChain$2([chunk, 'optionalAccess', _ => _.fileName]) || "");
}
function findChunkByName(name, bundle) {
    return Object.values(bundle).find(b => b.name && slash(b.name) === slash(name) && b.type === "chunk") ;
}
function findAssetByName(name, bundle) {
    return Object.values(bundle).find(b => b.name && slash(b.name) === slash(name) && b.type === "asset") ;
}
...
function replaceImportScriptPath(
    ...
    getScriptElems($)
        ...
        .attr("src", (i, value) => {
            ...
            const chunkName = getOutputFilenameFromChunk(resolve(basePath, value ), chunks);
...
const explorer$1 = cosmiconfigSync("manifest", {
    cache: false,
});
...
class ManifestProcessor {
...

 {
     ...
     */
     ...
     __init2() {this.readAssetAsBuffer = memoize(
        (filepath) => {
